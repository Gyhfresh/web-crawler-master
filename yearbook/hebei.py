...
def hebei(path):
    for year in range(2015, 2019):
        logger.info('现在爬取的是河北第' + str(year) + '年的统计年鉴')
        ##领域/不限
        url = 'http://tjj.shanxi.gov.cn/tjsj/tjnj/nj{}/indexce.htm'.format(year)
        ...
        for topic in topic_list:
            ...
            for file in file_list:
                if 'xls' in file.a.get('href') or 'xlrx' in file.a.get('href'):
                    ...
                    # print(click_name)
                    try:
                        nameyy = re.compile(r'html/(.*?)">', re.S)
                        namedown = re.findall(nameyy, str(file))
                        raw_file_name = namedown[0]
                        cur_file_name = click_name
                    except:

                        nameyy = re.compile(r'excel/(.*?)">', re.S)
                        namedown = re.findall(nameyy, str(file))
                        raw_file_name = namedown[0]
                        cur_file_name = click_name
                    ...
                    if os.path.exists(newpath) and not os.path.exists(path):
                        ...
                        while (True):
                            ...
                            if not os.path.exists(path):
                                break
                            if error_time >= 50:
                                logger.error('尝试多次无法找到' + str(newpath))
...
if __name__ == '__main__':
    path = os.path.dirname(__file__) + '/../info/河北年鉴.json'
